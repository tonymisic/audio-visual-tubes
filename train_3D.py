...
if record:
    ...
    wandb.run.name = "16 frames, 10k, 3D-HardWay + NP Loss"
    wandb.run.save()
    ...
    for epoch in range(args.epochs):
        if train:
            ...
            for step, (frames, spec, _, _, name) in enumerate(dataloader):
                ...
                heatmap, out = model(spec.float(), frames.float()) 
            ...
            with torch.no_grad():
                ...
                for step, (frames, spec, _, _, name) in enumerate(testdataloader):
                    ...
                    for step, i in enumerate(range(args.sampling_rate, frames.size(2) - 1, args.sampling_rate)):
                        heatmap_arr = heatmap[step].data.cpu().numpy()
